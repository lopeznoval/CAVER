...
class EB_RobotGUI_bis(QWidget):
    ...
    def _on_lora_message(self, msg: str):
        print(f"[DEBUG] Mensaje recibido: {msg}")  # Log principal para ver todos los mensajes
        self._append_input(msg)  # Opcional, para GUI si la usas

        try:
            data = json.loads(msg)
            if data.get("T") == 200 and data.get("action") == "capture_photo":
                print("[DEBUG] Comando de captura de foto recibido")  # Confirmación
                self.take_photo()
            else:
                print(f"[DEBUG] Mensaje ignorado: {data}")
        except Exception as e:
            print(f"[ERROR] Fallo al procesar mensaje: {e}")

    def take_photo(self):
        print("[DEBUG] Entrando en take_photo()")
        try:
            import io
            ...
            # time.sleep(1)  # deja que la cámara ajuste luz
            # camera.capture(stream, format='jpeg')
            # camera.close()
            # print("[DEBUG] Foto tomada correctamente")

            # Codificar en base64 y enviar de vuelta
            img_data = base64.b64encode(stream.getvalue()).decode('utf-8')
            response = {"T": 201, "image": img_data}
            self.send_cmd(json.dumps(response))
            print("[DEBUG] Foto enviada por LoRa")

        except Exception as e:
            print(f"[ERROR] Error en take_photo: {e}")
