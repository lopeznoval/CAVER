...
class RobotGUI:
    def __init__(self, master):
        ...
        self.feedback_running = False  # Estado de la retroalimentación continua
    ...
    def send_oled(self):
        try:
            ...
            messagebox.showerror("Error", "Line number must be between 0–3.")

    def send_cmd(self, cmd):
        ip = self.ip_entry.get().strip()
        if not ip:
            messagebox.showwarning("Warning", "Enter a valid IP address.")
            return
    ...
    def _send_request(self, ip, cmd):
        try:
            ...
            self._append_output(f"⚠️ Error: {e}\n")
    ...
    def start_feedback(self):
        ...
        self.send_cmd({"T": 131, "cmd": 1})  # Activar feedback continuo
    ...
    def stop_feedback(self):
        ...
        self.send_cmd({"T": 131, "cmd": 0})  # Desactivar feedback continuo
    ...
    def _feedback_loop(self):
        ...
        while self.feedback_running:
            try:
                ...
                self._append_output(f"[Feedback] {response}\n")
            except Exception as e:
                self._append_output(f"⚠️ Feedback error: {e}\n")
            time.sleep(1.0)  # Frecuencia de actualización (1 seg)
