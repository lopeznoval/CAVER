...
node = sx126x.sx126x(serial_num = "/dev/ttyAMA0",freq=433,addr=20,power=22,rssi=False,air_speed=2400,relay=False)
...
def send_deal():
    ...
    while True:
        ...
        if rec:
            ...
            # FLECHAS u otras secuencias ANSI (empiezan con ESC [)
            elif rec == '\x1b':
                seq = sys.stdin.read(2)  # lee los dos caracteres siguientes, p. ej. [A
                # Ignorar las flechas y secuencias de escape
                continue
    ...
    get_t = get_rec.split(",")
    if len(get_t) < 3:
        print("\n\033[1;31mInvalid format. Use for example: 0,868,Hello World\033[0m")
        return
